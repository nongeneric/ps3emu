...
CELL_OVIS_DEFINE_MAPPER(sec_rand_middle_square)
CELL_OVIS_DEFINE_MAPPER(sec_rand_park_and_miller)
...
void set_array(uint64_t obuf, int array[], char* mrand, char* msort)
{
	...
	if(array[0] == RAND_MIDDLE_SQUARE){
		if(cellOvisStartMapping_sec_rand_middle_square(obuf,MYTAG) == CELL_OK){
			cellOvisWaitMapping_sec_rand_middle_square(MYTAG);
		}
		for(j =0; j<N_ONEWAY; j++){
			ramdom_orderd_array[j] = rand_middle_square(mrand);
		}
	}else{
		if(cellOvisStartMapping_sec_rand_park_and_miller(obuf,MYTAG) == CELL_OK){
			cellOvisWaitMapping_sec_rand_park_and_miller(MYTAG);
		}
		for(j =0; j<N_ONEWAY; j++){
...
int cellSpursTaskMain(qword taskarg, uint64_t taskasetArg)
{ 
	(void) taskarg;
	uint64_t *args = (uint64_t *)memalign(128, sizeof(uint64_t) * 4);
	cellDmaGet((void*)args, taskasetArg, sizeof(uint64_t) * 4, MYTAG, 0 ,0);
	cellDmaWaitTagStatusAll(1<<MYTAG);
	
	uint64_t obuf = args[0];
	uint64_t ea_message = args[1];
	uint64_t ea_ramdom_orderd_array = args[2];
