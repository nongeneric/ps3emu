...
extern char _binary_spu_sample_ovis_auto_spu_elf_start[] ;
//elf_spuoverlay;
...
extern char *elf_test_overlay2_spu;
...
main(void)
{
  ...
  elf = (char*)_binary_spu_sample_ovis_auto_spu_elf_start;
  printf("elf=%p\n",elf);
  ...
  if (buf_top == NULL){
	  printf("## libovis : sample_ovis_auto_ppu FAILED ##\n");
    return -1;
  }
  ...
  if(ret != CELL_OK){
	  printf("cellOvisInitializeOverlayTable failed : return code = %d\n", ret);
	  printf("## libovis : sample_ovis_auto_ppu FAILED ##\n");
	  return -1;
  }
  ...
  r = system_ovis_spu(elf,
					  ...
					  "spu/sample_ovis_auto_spu.elf");
  if(r != CELL_OK){
	  printf("system_ovis_spu : return code = %d\n", r);
	  printf("## libovis : sample_ovis_auto_ppu FAILED ##\n");
	  return -1;
  }
  ...
  printf("## libovis : sample_ovis_auto_ppu SUCCEEDED ##\n");
  return 0;
}
