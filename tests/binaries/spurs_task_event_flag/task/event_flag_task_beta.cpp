...
int cellSpursTaskMain(qword argTask, uint64_t argTaskset)
{
	...
	/* notify to task alpha */
	unsigned int task_num = si_to_int(argTask);
	uint16_t mask = 0x01 << task_num;
	//spu_printf("[Task#%02u][SPU#%u] set event flag#0, mask = 0x%04x\n", idTask, idSpu, mask);
	ret = eventFlag0.set(mask);
	if (ret) {
		spu_printf("[Task#%02u][SPU#%u] cellSpursEventFlagSet failed : %x\n", idTask, idSpu, ret);
		abort();
	}

	/* wait for event flag#1 */
	//spu_printf("[Task#%02u][SPU#%u] waiting for event flag#0, mask = 0x%04x\n", idTask, idSpu, mask);
	ret = eventFlag1.wait(&mask, CELL_SPURS_EVENT_FLAG_AND);
	if (ret) {
		spu_printf("[Task#%02u][SPU#%u] cellSpursEventFlagWait failed : %x\n", idTask, idSpu, ret);
		abort();
	}
