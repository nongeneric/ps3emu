 ...
 *       6. Observe the performance by "observeRoundTripBySpuThread()".
...
#define LOG_FILE_NAME_PREFIX	"PPU-side_SPU_thread"

/*E The folloing function is defined in "observer.ppu.c" */
extern int observeRoundTripBySpuThread(void);
...
int main(void)
{
	...
	printf("----------------------------------\n");
	printf(" PPU-side Round Trip (SPU thread) \n");
	printf("----------------------------------\n");
	...
	ret = observeRoundTripBySpuThread();
	if (ret) {
		fprintf(stderr, "observeRoundTripBySpuThread() failed: %d\n", ret);
		cleanupMeasurementSpuThread();
		exit(6);
	}
	...
	printf(" \"PPU-side Round Trip (SPU thread)\" has finihsed successfully.\n");
	return 0;
}
