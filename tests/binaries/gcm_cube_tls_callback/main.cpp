...
int __thread tls_int = 200;

static void FlipCallback(uint32_t head)
{
	tls_int++;
	utilMonitorLock(&monitor, 0);
	printf("tls int: %d\n", tls_int);
    (void)head;
    sFlipped=true;
	utilMonitorSignal(&monitor);
	utilMonitorUnlock(&monitor);
}
...
static void flip(void)
{
	utilMonitorLock(&monitor, 0);
	sFlipped=false;
    cellGcmSetFlip(g_FrameIndex);
    cellGcmFlush();
    cellGcmSetWaitFlip();
	cellGcmFlush(gCellGcmCurrentContext);
	while(!sFlipped){
		utilMonitorWait(&monitor, 0);
	}
	utilMonitorUnlock(&monitor);
}
...
int32_t userMain()
{
  ...
  {
      GenerateTexture(g_TextureSys, g_TextureVid);
