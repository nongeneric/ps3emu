...
int main(void)
{
	 ...
	 */
	sys_event_queue_t queue, queue1;
	...
	ret = sys_event_queue_create(&queue1, &queue_attr, SYS_EVENT_QUEUE_LOCAL, 
								 QUEUE_SIZE);
	if (ret != CELL_OK) {
		fprintf(stderr, "sys_event_queue_create failed: %#.8x\n", ret);
		exit(ret);
	...
	ret = sys_spu_thread_group_connect_event_all_threads(group, queue1, 1ull << 58ull, &SPU_THREAD_PORT);
	if (SPU_THREAD_PORT != 58) {
		fprintf(stderr, "SPU_THREAD_PORT == %x\n", SPU_THREAD_PORT);
		return 1;
	}
	if (ret != CELL_OK) {
		fprintf(stderr, "sys_spu_thread_group_connect_event_all_threads failed: %#.8x\n", ret);
	}

	// destroying a queue automatically disconnects it from all threads
	sys_event_queue_destroy(queue1, SYS_EVENT_QUEUE_DESTROY_FORCE);

	ret = sys_spu_thread_group_connect_event_all_threads(group, queue, 1ull << 58ull, &SPU_THREAD_PORT);
	if (SPU_THREAD_PORT != 58) {
		fprintf(stderr, "SPU_THREAD_PORT == %x\n", SPU_THREAD_PORT);
		return 1;
	}
