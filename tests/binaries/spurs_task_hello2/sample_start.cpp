...
int main(void)
{
	...
	ret = cell::Spurs::SpursAttribute::initialize(&attribute, 1, SPURS_SPU_THREAD_PRIORITY, SPURS_PPU_THREAD_PRIORITY, false);
	if (ret) {
	...
	void* buffer = std::memalign(CELL_SPURS_TRACE_BUFFER_ALIGN, buffer_size);
	uint32_t mode = CELL_SPURS_TRACE_MODE_FLAG_WRAP_BUFFER |
		CELL_SPURS_TRACE_MODE_FLAG_SYNCHRONOUS_START_STOP;
	ret = cellSpursTraceInitialize(spurs, buffer, buffer_size, mode);
	if(ret == CELL_SPURS_CORE_ERROR_STAT) {
		/* Ignore this error, which is returned when the program is started from
		Tuner for PlayStation(R)3. Continue running the application without using
		the SPURS trace */
	} else if(ret != CELL_OK){
		return ret;
	}
