...
cellSpursTaskMain(qword argTask, uint64_t argTaskset)
{
	...
	//spu_printf ("SPU: iwl_event_flag_wait started... [%lld]\n", myNum);
	...
	/* */
	//spu_printf ("SPU: iwl_event_flag_wait waiting for notification... \n");
	uint16_t	mask = (0x0003) << ((myNum - 2) * 2);
	ret = cellSpursEventFlagWait (event_flag_spu_spu_ea, &mask, CELL_SPURS_EVENT_FLAG_AND);
	assert (ret == CELL_OK);

	//spu_printf ("SPU: iwl_event_flag_wait waked up... \n");
