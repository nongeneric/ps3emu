							   ...
							   const uint32_t row, const uint32_t col)
{
	...
	for (int r = 0; r <= rm; ++r) {
		for (int c = 0; c <= cm; ++c) {
			...
			point[p].pos[2] = 0;
			point[p].pos[3] = 1;
			point[p].u = x + 0.5 + 1.f/200;
			point[p].v = -y + 0.5 + 1.f/200;
...
void SampleApp::createFloatHeightTexture( void )
{
	...
	//mTexture.width = 2;
	//mTexture.height = 2;
	//mTexture.pitch = 2 * 4 * sizeof(float);

	for (int r = 0; r < sRow; ++r) {
		for (int c = 0; c < sColumn; ++c) {
			if (r > 50 && c > 50) {
				texture[4 * (r * sColumn + c)] = 1;
				texture[4 * (r * sColumn + c) + 1] = 0;
				texture[4 * (r * sColumn + c) + 2] = 0;
				texture[4 * (r * sColumn + c) + 3] = 1;
			} else if (c > 70 && r == 20) {
				texture[4 * (r * sColumn + c)] = 0;
				texture[4 * (r * sColumn + c) + 1] = 1;
				texture[4 * (r * sColumn + c) + 2] = 0;
				texture[4 * (r * sColumn + c) + 3] = 1;
			} else if (r < 20 && c == 20) {
				texture[4 * (r * sColumn + c)] = 1;
	...
	mTexture.pitch = sColumn * 4 * sizeof(float);
	...
	//mTexture.width = 10;
	//mTexture.height = 10;
	//mTexture.pitch = 10 * 4 * sizeof(float);
}
...
void SampleApp::onRender()
{
	...
	cellGcmSetVertexTextureBorderColor(mTexUnit, 0xFFFF00FF);
