...
unsigned char result[CELL_MD5_DIGEST_SIZE] __attribute__((__aligned__(16)));
CellMd5SpuInstance inst;


int md5_sum_spu(const char *mount, const char *file, CellSpursTaskset2 *taskset)
{
#define SHA_QDEPTH 8 
	...
	ret = cellMd5SpuCreateTask2(&inst, taskset, NULL, NULL, SHA_QDEPTH,DMA_BUFFER_SIZE);
	...
	do {
		...
		if (job_active[read_buf]){
			ret = cellMd5SpuCheckStatus(&inst, 1 << read_buf, 1);
		}
		...
		ret = cellMd5SpuProcessData(&inst, (uint64_t)((int)g_buf[read_buf]), nread, 1 << read_buf, 1);
		job_active[ read_buf ] = 1;

		read_buf = (read_buf + 1) % MAX_BUFF;
	} while (nread == FILEBUF_SIZE);

	ret=cellMd5SpuEndTask2(&inst, (uint64_t)result);
