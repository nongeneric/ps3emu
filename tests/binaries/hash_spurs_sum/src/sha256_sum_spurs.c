...
unsigned char result[CELL_SHA256_DIGEST_SIZE] __attribute__((__aligned__(16)));
CellSha256SpuInstance inst;
...
int sha256_sum_spu(const char *mount, const char *file, CellSpursTaskset2 *taskset)
{
	...
	ret = cellSha256SpuCreateTask2(&inst, taskset, NULL, NULL, SHA_QDEPTH,DMA_BUFFER_SIZE);
	if(ret){
		printf("cellSha256SpuCreateTask2() error 0x%x !\n", ret);
	}
	...
	do {
		...
		if (job_active[read_buf]){
			ret = cellSha256SpuCheckStatus(&inst, 1 << read_buf, 1);
		}
		...
		ret = cellSha256SpuProcessData(&inst, (uint64_t)((int)g_buf[read_buf]), nread, 1 << read_buf, 1);
		job_active[ read_buf ] = 1;

		read_buf = (read_buf + 1) % MAX_BUFF;
	} while (nread == FILEBUF_SIZE);
	ret=cellSha256SpuEndTask2(&inst, (uint64_t)result);

	printf("Hash result is: ");
	for (i = 0; i < CELL_SHA256_DIGEST_SIZE; i++){
		printf("%02x", result[i]);
	}
	printf("\n");
