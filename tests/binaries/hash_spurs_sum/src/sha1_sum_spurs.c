...
unsigned char result[CELL_SHA1_DIGEST_SIZE] __attribute__((__aligned__(16)));

CellSha1SpuInstance inst;

int sha1_sum_spu(const char *mount, const char *file, CellSpursTaskset2 *taskset)
{
	#define SHA_QDEPTH 8 
	...
	ret = cellSha1SpuCreateTask2(&inst, taskset, NULL, NULL, SHA_QDEPTH,DMA_BUFFER_SIZE);
	...
	do {
		...
		if (job_active[read_buf]){
			ret = cellSha1SpuCheckStatus(&inst, 1<<read_buf, 1);
		}
		...
		ret = cellSha1SpuProcessData(&inst, (uint64_t)((int)g_buf[read_buf]), nread, 1<<read_buf, 1);
		job_active[ read_buf ]=1;

		read_buf = (read_buf + 1) % MAX_BUFF;
	} while (nread == FILEBUF_SIZE);

	ret=cellSha1SpuEndTask2(&inst, (uint64_t)result);

	printf("Hash result is: ");
	for (i = 0; i < CELL_SHA1_DIGEST_SIZE; i++){
		printf("%02x", result[i]);
	}
	printf("\n");
