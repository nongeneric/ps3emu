...
unsigned char result[CELL_SHA224_DIGEST_SIZE] __attribute__((__aligned__(16)));

CellSha224SpuInstance inst;


int sha224_sum_spu(const char *mount, const char *file, CellSpursTaskset2 *taskset)
{
	#define SHA_QDEPTH 8 
	...
	ret = cellSha224SpuCreateTask2(&inst, taskset, NULL, NULL, SHA_QDEPTH,DMA_BUFFER_SIZE);
	...
	do {
		...
		if (job_active[read_buf]){
			ret = cellSha224SpuCheckStatus(&inst, 1 << read_buf, 1);
		}
		...
		ret = cellSha224SpuProcessData(&inst, (uint64_t)((int)g_buf[read_buf]), nread, 1 << read_buf, 1);
		job_active[ read_buf ]=1;

		read_buf = (read_buf + 1) % MAX_BUFF;
	} while (nread == FILEBUF_SIZE);
	ret=cellSha224SpuEndTask2(&inst, (uint64_t)result);

	printf("Hash result is: ");
	for (i = 0; i < CELL_SHA224_DIGEST_SIZE;i++){
		printf("%02x", result[i]);
	}
	printf("\n");
