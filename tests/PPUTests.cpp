...
TEST_CASE("ppu_lwarx_should_not_destroy_reservations_of_other_threads") {
    ...
    std::atomic<bool> th2_step1 = false;
...
TEST_CASE("emu_vspltw_v19_v16_2") {
    MainMemory mm;
    g_state.mm = &mm;
    PPUThread th;
    // UIMM = 30
    uint8_t instr[] = { 0x12, 0x7e, 0x82, 0x8c };
    th.r(16).set_xmm(_mm_set_epi32(1, 2, 3, 4));
    ppu_dasm<DasmMode::Emulate>(instr, 0, &th);
    REQUIRE( th.r(19).w(0) == 3 );
    REQUIRE( th.r(19).w(1) == 3 );
    REQUIRE( th.r(19).w(2) == 3 );
    REQUIRE( th.r(19).w(3) == 3 );
}