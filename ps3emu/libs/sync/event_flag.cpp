...
int32_t sys_event_flag_create(big_uint32_t* id,
    ...
    flag->value = init;
    *id = map.create(flag);
    INFO(sync) << ssnprintf("sys_event_flag_create(%x, %x, %s)", *id, init, std::string(attr->name, 8));
    return CELL_OK;
}
...
int32_t sys_event_flag_set(uint32_t id, uint64_t bitptn) {
    INFO(sync) << ssnprintf("sys_event_flag_set(%x, %llx)", id, bitptn);
    auto flag = map.get(id);
    boost::unique_lock<boost::mutex> lock(flag->m);
    flag->value |= bitptn;
    flag->cv.notify_all();
...
int32_t sys_event_flag_get(uint32_t id, big_uint64_t* value) {
    INFO(sync) << ssnprintf("sys_event_flag_set(%x)", id);
    auto flag = map.get(id);
    boost::unique_lock<boost::mutex> lock(flag->m);
    *value = flag->value;
    return CELL_OK;
